...
try:
    ...
    print("ORCA JobCheck Utility version", scriptversion, "(Python version)", )
    print("---------------------------------")
    print("Script usage:")
...
#Here checking for stuff in listed inputfile until 'END OF INPUT'
            if 'INPUT LINE' != "unset" and endofinput=="unset":
                ...
                if "$new_job" in line and newjob == "unset":
                    newjob="yes"
                    print("New_job feature detected! Script will probably not deal with multi-job outputfiles correctly! Complain to RB.",)
            ...
            if jobtype=="sp" or jobtype=="freqsp" and postHF=="unset":
                if jobtype=="freqsp":
                    if len(imaginmodes)==0:
                        print('{0:40}   {1:40}'.format( filename+":",  scfenergy), )
                    else:
                        print('{0:40}   {1:10}   {2:40}'.format( filename+":",  scfenergy,  "Imaginary modes"), )
                if jobtype=="sp":
                    if scfconv=="yes" or casscfconv=="yes":
                        print('{0:40}   {1:40}'.format( filename+":",  scfenergy), )
                    else:
                        print('{0:40}   {1:40}'.format( filename+":",  "Not converged!"), )
            elif jobtype=="sp" and postHF!="unset":
                print('{0:40}   {1:40}'.format( filename+":",  scfenergy), )
            elif optjob=="yes" or jobtype=="optfreq" or jobtype=="opttsfreq":
                if optrunconverged=="yes":
                    #print("optenergy is", optenergy)
                    print('{0:40}   {1:40}'.format( filename+":",  str(optenergy)), )
                else:
                    print('{0:40}   {1:40}'.format( filename+":",  "Optimization failed!"), )
        elif orcacrash=="yes":
            print('{0:40}   {1:40}'.format( filename+":",  "ORCA Crash!"), )
        else:
            print('{0:40}   {1:40}'.format( filename+":",  "Running?"), )
# LONG PRINTING MODE
    else:
        print("")
        print("ORCA JobCheck Utility modified version", scriptversion, "(Python3 version)", )
        print("-----------------------------------------------------------------------")
        print("File:", filename, )
        if parproc!="unset":
        ...
        if runcomplete=="yes":
            print("ORCA terminated normally (",' '.join(runtime),")", )
        elif orcacrash=="yes":
            if optrunconverged=="yes":
                print("Optimization converged! in (", optcycle, "iterations). YAY!", )
                print( "FINAL OPTIMIZED ENERGY:", finaloptenergy, )
            print("ORCA JOB Crashed!", )
            print("Error message:")
            ...
            try:
                if optjob=="yes":
                    ...
                    if sys.argv[2]=="-plotgrad":
                        print("Plotting Gradient in Matplotlib...", )
                    ...
                    if sys.argv[2]=="-plotstep":
                        print("Plotting Step in Matplotlib...", )
                        allcycles=list(range(1, optcycle+1))
                        import subprocess
                        import matplotlib.pyplot as plt
                    ...
                    if sys.argv[2]=="-plotenergy":
                        print("Plotting Energy in Matplotlib...", )
                        allcycles=list(range(1, optcycle+1))
                        import subprocess
                        import matplotlib.pyplot as plt
            ...
            #print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
            if xyzfileerror=="yes":
                print("Fatal error: Job could not open xyz file", )
            if cpscferror=="yes":
                print("CPSCF error", )
        else:
            print("ORCA has not terminated with message and may still still be running this job", )
        ...
        if earlycrash=="yes":
            ...
            if jobtype=="scan":
                ...
                if len(scanenergies)==0:
                    print("Still running first scan step ...", )
                    #break
                else:
                    count=0
            ...
            if (jobtype=="sp" or jobtype=="freqsp") and postHF=="unset":
                ...
                if runcomplete=="yes" and scfconv=="yes":
                    print("SCF CONVERGED AFTER", scfcycles, "CYCLES", )
                    print("HOMO-LUMO gap (alpha) is:", gap_a, "eV")
                    if scftype=="UHF":
                        print("SCF type is", scftype, " S**2:", s2value, " Ideal value:", ideals2value)
                    print("FINAL SINGLE POINT ENERGY IS", scfenergy, ) 
                    if dft=="yes":
                        print("Integrated no. electrons:", intelectrons, "(should be", actualelec, ")")
                elif runcomplete=="yes" and scfconv=="no":
                    print("SCF DID NOT CONVERGE in", unfinscfcycles, "cycles. Check your SCF settings.", )
                    if scfalmostconv=="yes":
                        print("SCF was close to convergence though (\"signs of convergence\").", )
                        print("Energy is", scfenergy, )
                elif runcomplete=="yes" and noiter=="yes":
                    print("SCF mode: No iterations", )
                    print("FINAL SINGLE POINT ENERGY IS", scfenergy, )
                    if dft=="yes":
                        print("Integrated no. electrons:", intelectrons, "(should be", actualelec, ")")
                #Runcomplete not true but SCFconv is yet. Probably freqsp job that failed in freq step
                    ...
                    if finalsingleline==True:
                        print("FINAL SINGLE POINT ENERGY IS", scfenergy, )
                    if orcacrash=="yes":
                        print("Freqjob failed", )
                    else:
                        print("Job still running", )
                elif scferrorgeneral=="yes":
                    print("SCF has crashed. Sad...", )
                elif scfstillrunning=="yes":
                    print("SCF is probably still be running", )
                elif casscf=="yes" and casscfconv=="yes":
                    print("CASSCF CONVERGED AFTER", lastmacroiter, "CYCLES", )
                    ...
                    if casscf=="yes":
                        print("CASSCF still running?", )
                    else:
                        print("SCF still running?", )
            if jobtype=="sp" and postHF=="yes":
                ...
                if runcomplete=="yes" and scfconv=="yes" and extrapolate!="yes":
                    ...
                    print("SCF is done.", )
                    print("SCF energy is:", purescfenergy, )
                    print("Running post-HF step", )
                elif extrapolate=="yes" and runcomplete!="yes":
                    if orcacrash !="yes":
                        print(WARNING+"This is a running extrapolation job",)
                    else:
                        print(FAIL+"This was an extrapolation job that crashed",)
                    ...
                    print("Final extrapolated total energy:", scfenergy, )
                    print("")
                else:
                    print("SCF still running?", )
            ...
            if optjob=="yes":
                if optrunconverged=="yes":
                    ...
                    if jobtype=="optfreq":
                        if freqsection!="done":
                            print("Frequency job did not finish", )
                    if dft=="yes":
                        print("Integrated no. electrons:", intelectrons, "(should be", actualelec, ")")
                elif optnotconv=="yes":
                    print("Optimization did not converge in", optcycle, "optimization steps", )
                    ...
                    if optrunconverged!="yes":
                        ...
                        if freqsection=="done":
                            if len(imaginmodes)==1:
                                print("We have 1 imaginary mode (",imaginmodes[0], "cm^-1) for saddlepoint. Good!", )
                            if len(imaginmodes)==0:
                                print("We have no imaginary modes for saddlepoint. Bad...", )
                            if len(imaginmodes)>1:
                                print("We have many imaginary modes for saddlepoint. Bad...", )
                        else:
                            print("Frequency job did not finish", )
                if jobtype=="optfreq" or jobtype=="freqsp":
                    if optrunconverged!="yes":
                        if jobtype=="freqsp":
                            ...
                            if freqsection=="done":
                                if len(imaginmodes)==1:
                                    print("We have 1 imaginary mode for minimum. Bad...:", imaginmodes[0], )
                                elif len(imaginmodes)==0:
                                    print( "We have no imaginary modes for minimum. Good. Lowest mode is", lowestvib, )
                                    if float(lowestvib) < 0:
                                        print( "Probably some numerical noise present, however.", )
                                elif len(imaginmodes)>1:
                                    print("Wes have several imaginary modes for minimum. Bad...", )

                        elif jobtype=="optfreq":
                            print("Optimization did not finish properly.",)
                        else:
                            print("Frequency job did not finish", )
                    else:
                        if freqsection=="done":
                            if len(imaginmodes)==1:
                                ...
                                print("We have several imaginary modes for minimum. Bad...", )
                        else:
                            print("Frequency job did not finish", )
                try:
                    ...
                    if sys.argv[2]=="-l":
                        ...
                        with open(filename, errors='ignore') as cfile:
                            ...
                            for dline in reverse_lines(cfile):
                                ...
                                bla.append(dline.strip('\n'))
                                if scount==nlines:
                                    print("Last", nlines, "lines of output:",)
                                    for bline in reversed(bla):
                                        print(bline),
                                    break
                    if sys.argv[2]=="-grad":
